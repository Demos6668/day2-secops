...
(async () => {
  ...
  try {
    ...
    // Locate iframe by title or url
    let iframeEl = await page2.$('iframe[title="Day2 OSINT"]');
    if (!iframeEl) {
      // fallback: any iframe pointing at 5173
      const frames = page2.frames();
      const targetFrame = frames.find(f => f.url().includes('5173'));
      if (targetFrame) {
        const viImgFrame = await targetFrame.$('img[src*="myvi.in"]');
        const viNWFrame = viImgFrame ? await viImgFrame.evaluate(el => el.naturalWidth) : 0;
        results.check2_iframe_vi_img = !!viImgFrame;
        results.check2_iframe_vi_natural_width = viNWFrame;
      } else {
        results.check2_iframe_vi_img = 'IFRAME NOT FOUND';
      }
    } else {
      const iframeContentFrame = await iframeEl.contentFrame();
      if (iframeContentFrame) {
        const viImgFrame = await iframeContentFrame.$('img[src*="myvi.in"]');
        const viNWFrame = viImgFrame ? await viImgFrame.evaluate(el => el.naturalWidth) : 0;
        results.check2_iframe_vi_img = !!viImgFrame;
        results.check2_iframe_vi_natural_width = viNWFrame;
      }
    ...
    // Screenshot of the 5174 page (shows iframe area)

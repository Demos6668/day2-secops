...
// ---------------------------------------------------------------------------
test('Check 1 dark: glass panels have dark #161B22 background, hairline borders', async ({ page }) => {
...
test('Check 1 light: glass panels have white background, hairline borders visible', async ({ page }) => {
...
// ---------------------------------------------------------------------------
test('Check 2 light: dangerous-class rows show dark red text (#B91C1C) not faint pink', async ({ page }) => {
  await setTheme(page, 'light');
...
// ---------------------------------------------------------------------------
test('Check 3 light: in-progress amber chips show dark amber (#B45309) not faint orange', async ({ page }) => {
  await setTheme(page, 'light');
...
// ---------------------------------------------------------------------------
test('Check 4 light: loss-reason chips have readable text on /tools/impervaWaf', async ({ page }) => {
  await setTheme(page, 'light');
  ...
  const chips = page.locator('[class*="B91C1C"], [class*="B45309"], [class*="92400E"], [class*="15803D"]');
...
// ---------------------------------------------------------------------------
test('Check 5 contrast: B91C1C element computes rgb(185,28,28) not the pink tint in light mode', async ({ page }) => {
  await setTheme(page, 'light');
  await page.goto(`${BASE}/`);
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(800);
  ...
  console.log('Computed color for text-[#B91C1C] dark:text-[#F87171] in light mode:', computedColor);
...
// ---------------------------------------------------------------------------
test('Check 6 dark regression: severity chips render original light tints (F87171, F59E0B, etc.)', async ({ page }) => {
  await setTheme(page, 'dark');
  await page.goto(`${BASE}/`);
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(800);
